	...
	def __init__(self, dataframe, num_normalizer="quantile", seed=0):
		...
		for i, col in enumerate(self.columns):
			if col in self.num_normalizer.keys():
				self.norm_data.append(self.num_normalizer[col].transform(self.reorder_data[:,i].reshape(-1,1).astype(np.float32)))
			else:
...
def generate_pk_weights(table, pk_name, removal_attr, removal_attr_values, attr_type="Numerical"):
    ...
    if attr_type == "Numerical":
        ...
        select_count = project_tb.loc[project_tb[removal_attr].isin(removal_attr_values)].groupby(pk_name).count().reset_index()
